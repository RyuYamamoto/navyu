...
def generate_launch_description():
    ...
    map_file_path = PathJoinSubstitution([FindPackageShare("navyu_navigation"), "map", "map.yaml"])

    nav2_config_path = PathJoinSubstitution(
        [FindPackageShare("navyu_navigation"), "config", "nav2_params.yaml"]
    )

    nav2_launch_path = PathJoinSubstitution([FindPackageShare("nav2_bringup"), "launch"])

    default_bt_xml_filename = PathJoinSubstitution(
        [
            FindPackageShare("nav2_bt_navigator"),
            "behavior_trees",
            "navigate_w_resplanning_and_recovery.xml",
        ]
    )
